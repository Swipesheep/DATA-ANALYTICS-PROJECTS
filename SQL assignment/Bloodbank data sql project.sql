...
--        SQL Project with Schema, Data & Queries
-- ============================================================
...
CREATE TABLE BloodTypes (
    ...
    blood_group     VARCHAR(5) NOT NULL UNIQUE,   -- e.g. A+, A-, O+, AB-
...
CREATE TABLE Donors (
    ...
    address         VARCHAR(255),
    city            VARCHAR(100),
    weight_kg       DECIMAL(5,2),              -- donors must be >50 kg
    is_active       BOOLEAN DEFAULT TRUE,       -- eligible to donate
    registered_at   TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (blood_type_id) REFERENCES BloodTypes(blood_type_id)
);

-- Donor Health Screening (checked at each donation)
CREATE TABLE DonorHealthScreening (
    screening_id        INT PRIMARY KEY AUTO_INCREMENT,
    donor_id            INT NOT NULL,
    screening_date      DATE NOT NULL,
    hemoglobin_level    DECIMAL(4,1),          -- g/dL  (min 12.5 for females, 13.0 males)
    blood_pressure_sys  INT,                   -- systolic mmHg
    blood_pressure_dia  INT,                   -- diastolic mmHg
    pulse_rate          INT,                   -- bpm
    temperature_f       DECIMAL(4,1),          -- °F
    ...
    is_eligible         BOOLEAN DEFAULT TRUE,   -- cleared to donate
    notes               TEXT,
...
CREATE TABLE Donations (
    ...
    units_donated       DECIMAL(4,2) DEFAULT 1.00,  -- 1 unit ≈ 450 ml
...
CREATE TABLE BloodInventory (
    ...
    expiry_date         DATE NOT NULL,          -- Whole Blood: 42 days; Platelets: 5 days
...
-- Blood Requests (from hospitals or recipients)
...
-- Blood Transfusions (fulfillment of requests)
...
('Nikhil',   'Pawar',   '1975-12-25', 'Male',   6,  '7980606119', 'N.pawar@mail.com',    'Kolkata',  82.0,  FALSE), -- inactive
...
(donor_id, screening_date, hemoglobin_level, blood_pressure_sys, blood_pressure_dia, pulse_rate, temperature_f,
 has_hiv, has_hepatitis_b, has_hepatitis_c, has_syphilis, has_malaria, is_eligible, notes, screened_by)
VALUES
...
(7,  '2024-02-05', 14.0, 120, 78, 74, 98.6, FALSE, TRUE,  FALSE, FALSE, FALSE, FALSE, 'Hepatitis B positive — deferred', 'Dr. Saha'),  -- ineligible
...
(10, '2024-02-15', 13.8, 114, 71, 69, 98.6, FALSE, FALSE, TRUE,  FALSE, FALSE, FALSE, 'Hepatitis C detected — deferred', 'Dr. Saha'), -- ineligible
...
-- Additional screenings (repeat donors)
...
-- donor 7 was ineligible — no donation
(8,  8,  '2024-02-08', 1.00, 'Whole Blood', 'Nurse kaur'),
(9,  9,  '2024-02-10', 1.00, 'Whole Blood', 'Nurse Tamang'),
-- donor 10 was ineligible — no donation
(11, 11, '2024-03-01', 1.00, 'Plasma',      'Nurse kaur'),
(12, 12, '2024-03-05', 1.00, 'Whole Blood', 'Nurse Tamang'),
(1,  13, '2024-06-15', 1.00, 'Whole Blood', 'Nurse kaur'),   -- repeat donation
(3,  14, '2024-07-20', 1.00, 'Whole Blood', 'Nurse Tamang');   -- repeat donation

-- Blood Inventory (expiry: Whole Blood +42d, Plasma +365d, Platelets +5d)
...
VALUES
(1,  7, 'Whole Blood', 1.00, '2024-01-10', '2024-02-21', 'Fridge-A1', FALSE, 'Used'),       -- O+
(2,  1, 'Whole Blood', 1.00, '2024-01-12', '2024-02-23', 'Fridge-A2', FALSE, 'Used'),        -- A+
(3,  3, 'Plasma',      1.00, '2024-01-15', '2025-01-15', 'Freezer-B1', FALSE, 'Available'),  -- B+
(4,  8, 'Whole Blood', 1.00, '2024-01-18', '2024-02-29', 'Fridge-A3', FALSE, 'Expired'),     -- O-
(5,  5, 'Platelets',   1.00, '2024-01-20', '2024-01-25', 'Fridge-C1', FALSE, 'Expired'),     -- AB+
(6,  2, 'Whole Blood', 1.00, '2024-02-01', '2024-03-14', 'Fridge-A4', FALSE, 'Used'),        -- A-
(7,  4, 'Whole Blood', 1.00, '2024-02-08', '2024-03-21', 'Fridge-A5', TRUE,  'Discarded'),   -- B-  contaminated
(8,  7, 'Whole Blood', 1.00, '2024-02-10', '2024-03-23', 'Fridge-A6', FALSE, 'Used'),        -- O+
(9,  6, 'Plasma',      1.00, '2024-03-01', '2025-03-01', 'Freezer-B2', FALSE, 'Available'),  -- AB-
(10, 7, 'Whole Blood', 1.00, '2024-03-05', '2024-04-16', 'Fridge-A7', FALSE, 'Available'),   -- O+
(11, 7, 'Whole Blood', 1.00, '2024-06-15', '2024-07-27', 'Fridge-A8', FALSE, 'Available'),   -- O+
(12, 3, 'Whole Blood', 1.00, '2024-07-20', '2024-08-31', 'Fridge-B3', FALSE, 'Available');   -- B+
...
(5, 4, 8, 'Platelets',   2.00, 'Critical', '2024-02-08', '2024-02-09', 'Rejected',  'Dr. Patel'),  -- no O- platelets
...
(NULL, 3, 7, 'Whole Blood',3.00,'High',    '2024-07-18', '2024-07-20', 'Pending',   NULL),   -- hospital request (no specific recipient)
...
(3, 3,  3, 3, '2024-02-10', 1.00, 'Dr. Kumar',   'Successful'),  -- plasma used for B+
...
-- ─────────────────────────────────────────────────────────
-- Q1. Is there sufficient blood available at the blood bank?
--     (Current available units by blood group and type)
-- ─────────────────────────────────────────────────────────
SELECT
    bt.blood_group,
    bi.donation_type,
...
-- ─────────────────────────────────────────────────────────
-- Q2. Total stock summary vs. pending requests
...
SELECT
    ...
    COALESCE(d.requested_units, 0)               AS pending_request_units,
    COALESCE(s.available_units, 0)
        - COALESCE(d.requested_units, 0)         AS net_balance,
    CASE
        WHEN COALESCE(s.available_units, 0) < COALESCE(d.requested_units, 0)
            THEN 'SHORTAGE ⚠️'
        WHEN COALESCE(s.available_units, 0) = 0
            THEN 'OUT OF STOCK 🚫'
        ELSE 'Sufficient ✅'
    END AS stock_status
FROM Supply s
FULL OUTER JOIN Demand d ON s.blood_group = d.blood_group
ORDER BY net_balance;


-- ─────────────────────────────────────────────────────────
-- Q3. Does the donor have any blood-borne diseases?
--     (List all donors flagged with any disease)
-- ─────────────────────────────────────────────────────────
SELECT
    d.donor_id,
    ...
    bt.blood_group,
    dhs.screening_date,
    CASE WHEN dhs.has_hiv         THEN 'HIV '         ELSE '' END ||
    CASE WHEN dhs.has_hepatitis_b THEN 'Hepatitis-B '  ELSE '' END ||
    CASE WHEN dhs.has_hepatitis_c THEN 'Hepatitis-C '  ELSE '' END ||
    CASE WHEN dhs.has_syphilis    THEN 'Syphilis '     ELSE '' END ||
    CASE WHEN dhs.has_malaria     THEN 'Malaria '      ELSE '' END  AS detected_diseases,
    dhs.is_eligible,
    dhs.notes
...
--     (Urgent — find recipients quickly)
-- ─────────────────────────────────────────────────────────
SELECT
    bi.inventory_id,
    bt.blood_group,
...
--     (Include time since last donation — must be 56+ days)
-- ─────────────────────────────────────────────────────────
SELECT
    d.donor_id,
    CONCAT(d.first_name, ' ', d.last_name) AS donor_name,
...
SELECT
    ...
    ROUND(
        100.0 * SUM(CASE WHEN br.status = 'Fulfilled' THEN 1 ELSE 0 END) / COUNT(br.request_id),
...
-- ─────────────────────────────────────────────────────────
-- Q11. Blood group compatibility check for a recipient
--      (Which inventory units can be given to a patient?)
--      Example: Find compatible blood for a B+ recipient
-- ─────────────────────────────────────────────────────────
-- Compatibility rules encoded inline:
...
WHERE bi.status = 'Available'
  AND bi.is_contaminated = FALSE
  AND bi.expiry_date >= CURDATE()
  AND bt_inv.blood_group IN (
      -- Replace 'B+' with target recipient's blood group
      SELECT compatible_group FROM (
          VALUES ('B+'), ('B-'), ('O+'), ('O-')    -- compatible donors for B+ recipient
      ) AS compat(compatible_group)
  )
ORDER BY bi.expiry_date ASC;
...
GROUP BY d.donor_id, donor_name, bt.blood_group, d.is_active
...
--      (Donors who were screened but deemed ineligible)
-- ─────────────────────────────────────────────────────────
SELECT
    CONCAT(d.first_name, ' ', d.last_name) AS donor_name,
    bt.blood_group,
    ...
    CASE WHEN dhs.has_hiv         THEN 'YES' ELSE 'NO' END AS HIV,
    CASE WHEN dhs.has_hepatitis_b THEN 'YES' ELSE 'NO' END AS Hepatitis_B,
    CASE WHEN dhs.has_hepatitis_c THEN 'YES' ELSE 'NO' END AS Hepatitis_C,
    CASE WHEN dhs.has_syphilis    THEN 'YES' ELSE 'NO' END AS Syphilis,
    CASE WHEN dhs.has_malaria     THEN 'YES' ELSE 'NO' END AS Malaria,
    dhs.screened_by
FROM DonorHealthScreening dhs
JOIN Donors d   ON dhs.donor_id = d.donor_id
JOIN BloodTypes bt ON d.blood_type_id = bt.blood_type_id
...
--      (screening → donation → inventory in one call)
-- ─────────────────────────────────────────────────────────
DELIMITER $$
...
BEGIN
    ...
    IF p_hgb < 12.5 THEN SET v_eligible = FALSE; END IF;
    ...
    VALUES
        (p_donor_id, CURDATE(), p_hgb, p_bp_sys, p_bp_dia,
         p_pulse, p_temp, p_has_hiv, p_has_hep_b, p_has_hep_c,
    ...
    ELSE
        -- Insert donation
        INSERT INTO Donations (donor_id, screening_id, donation_date, units_donated, donation_type, collected_by)
        VALUES (p_donor_id, v_screening_id, CURDATE(), 1.00, p_donation_type, p_collected_by);
        ...
        SET p_message = CONCAT('Donation registered successfully. Inventory ID: ', LAST_INSERT_ID());
    END IF;
END$$
DELIMITER ;
...
-- ─────────────────────────────────────────────────────────
CREATE OR REPLACE VIEW vw_BloodAvailability AS
SELECT
    bt.blood_group,
    bi.donation_type,
...
-- ─────────────────────────────────────────────────────────
CREATE OR REPLACE VIEW vw_DonorEligibility AS
SELECT
    d.donor_id,
    CONCAT(d.first_name, ' ', d.last_name) AS donor_name,
    bt.blood_group,
    ...
    (SELECT is_eligible FROM DonorHealthScreening
     WHERE donor_id = d.donor_id
     ORDER BY screening_date DESC LIMIT 1)          AS last_screening_eligible,
    CASE
        WHEN d.is_active = FALSE THEN 'Inactive'
        WHEN (SELECT is_eligible FROM DonorHealthScreening
              WHERE donor_id = d.donor_id
              ORDER BY screening_date DESC LIMIT 1) = FALSE THEN 'Deferred - Disease Detected'
        WHEN DATEDIFF(CURDATE(), MAX(don.donation_date)) < 56 THEN 'Too Soon'
        ELSE 'Eligible'
    END AS eligibility_status
FROM Donors d
JOIN BloodTypes bt ON d.blood_type_id = bt.blood_type_id
...
-- ============================================================
-- END OF BLOOD BANK MANAGEMENT SYSTEM SQL PROJECT
-- ============================================================